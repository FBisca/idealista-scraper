...
export class IdealistaListParserPlugin extends ContentParserPlugin<
  string,
  IdealistaListParseResult
> {
  ...
  public async extract(
    ...
    $('article.item').each((_, element) => {
      ...
      const currentPrice = this.normalizeText(
        article.find('.item-price').first().text(),
      );
      ...
      const description = this.optionalText(
        article.find('.item-description, .item-paragraph').first().text(),
      );
      const tags = this.extractTags(article, $);
      const originalPrice = this.optionalText(
        article
          .find('.pricedown_price, .item-price-discount, .item-price--discount')
          .first()
          .text(),
      );
      const price = this.buildPriceInfo(currentPrice, originalPrice);
      const includedParking =
        article.find('.price-row .item-parking').length > 0;
      const agencyCandidates = this.collectAgencyCandidates(
        article,
        content.url,
        $,
      );
    ...
    return {
      ...
      pagination: {
        currentPage,
        ...(nextPageHref
          ? { nextPageUrl: this.toAbsoluteUrl(nextPageHref, content.url) }
          : {}),
      },
  ...
  private extractCurrentPage($: cheerio.CheerioAPI): number {
    const currentPageText = this.optionalText(
      $('.pagination li.selected span').first().text(),
    );
    if (!currentPageText) {
      return 1;
    }
  ...
  private extractTotalItems($: cheerio.CheerioAPI): number | undefined {
    const text = this.optionalText(
      $('.breadcrumb-navigation-element-info').first().text(),
    );
    if (!text) {
      return undefined;
    }
  ...
  private extractAveragePricePerSquareMeter(
    $: cheerio.CheerioAPI,
  ): IdealistaAveragePricePerSquareMeter | undefined {
  ...
  private extractTags(
    ...
    const tags = article
      .find(
        '.item-tag, .item-label, .tag, .listing-tags, .listing-tags-container .listing-tags',
      )
  ...
  private collectAgencyCandidates(
    ...
    for (const selector of selectors) {
      article.find(selector).each((_, element) => {
        ...
        const hrefRaw = this.optionalText(
          link.attr('href') ?? closestAnchor.attr('href'),
        );
        const title = this.optionalText(
          link.attr('title') ?? closestAnchor.attr('title'),
        );
        ...
        const isListingLink = Boolean(
          className?.includes('item-link') || hrefRaw?.includes('/inmueble/'),
        );
        if (isListingLink) {
          return;
        }
        ...
        const href = hrefRaw
          ? this.toAbsoluteUrl(hrefRaw, sourceUrl)
          : undefined;
  ...
  private resolveAgencyFromCandidates(
    candidates: IdealistaAgencyDebugCandidate[],
    ...
    const markupFromSameHref = preferredCandidate.href
      ? candidates.find(
          (candidate) =>
            candidate.href === preferredCandidate.href && candidate.markup,
        )?.markup
      : undefined;
    ...
    const title =
      ...
      this.inferAgencyTitleFromHref(preferredCandidate.href);
  ...
  private buildPriceInfo(
    ...
    const originalValue = originalPrice
      ? this.parsePriceValue(originalPrice)
      : undefined;
